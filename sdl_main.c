...
void update_scrn(void) {
	...
	if (!chromamode) 
	{
		...
		if (sdl_emulator.invert)
		{
			...
			{
				for (x = 0; x < ZX_VID_X_WIDTH; x += 8)
				{
					*v64ptr++ = lookupv[*ptr++];
				}
				ptr += ((ZX_VID_FULLWIDTH - ZX_VID_X_WIDTH) / 8);
			}
		}
		else
		{
	...
	{
		unsigned char* ptr = scrnbmpc + ZX_VID_X_YOFS * ZX_VID_FULLWIDTH / 8 + ZX_VID_X_XOFS / 8;
		uint8_t* vcptr = vptr;

		for (y = 0; y < ZX_VID_X_HEIGHT; y++)
		{
			for (x = 0; x < ZX_VID_X_WIDTH; x++)
			{
				*vcptr++ = *ptr++;
			}
			ptr += ((ZX_VID_FULLWIDTH - ZX_VID_X_WIDTH) / 8);
